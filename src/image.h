...
struct ImageT {
   ...
   public:
    ...
    inline int chan() const { return BPP / (sizeof(T) * 8); }  // BPP 表示每像素的位数，通过将 BPP 除以 sizeof(T) * 8 得到通道数。例如，如果 T 是 unsigned char（通常为 1 字节），BPP 为 24，则 chan() 会返回 3，因为 24 位每像素通常表示 3 个 8 位通道（如 RGB）
